 ...
 * to {@link #send(Completed)} until the previous call completes.
 * <p>
...
public interface BoundedAsyncChannel<T> extends AsyncIterator<T> {
   ...
   * This channel is bounded, so after a call to {@code send} a future is returned to the sender.
   * When the future completes, consumption has progressed enough that the channel is again willing
   * to accept messages. The implementation may decide when a channel is writable, it could require
   * that all outstanding values are consumed by the consumer, it could allow a certain number of
   * values to be buffered before applying back pressure, or it could use some out of band metric to
   * decide.
   * <p>
   * Note that {@link #close()} is the <b>only</b> way to close the channel. Specifically,
   ...
   *         completes with true if the item was accepted, false if it was rejected because the
   ...
   * Optional.empty(). When the {@link Future} returned by this method completes, no more messages
   * will ever make it into the channel. Equivalently, all {@code true} futures generated by calls
   * to {@link #send(Completed)} will have been completed by the time the returned future completes.
   * <p>
   * Note that {@link #close()} is the <b>only</b> way to close the channel. Specifically,
   * exceptions don't close the channel. This is consistent with the interface on

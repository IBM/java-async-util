...
public interface BoundedAsyncChannel<T> extends AsyncIterator<T> {
   ...
   * a channel is writable, it could require that allOf outstanding values are consumed by the
   ...
   * will fail.
   *
   * <p>After the channel is terminated, allOf subsequent sends will return stages that will complete
   ...
   * channel. Equivalently, allOf stages generated by {@link #send} that will complete with {@code
   * true} generated by calls to {@link #send} will have been completed by the time the returned
   * stage completes.
   *
   * @return a {@link CompletionStage} that indicates when allOf sends that were sent before the {@link #terminate()}

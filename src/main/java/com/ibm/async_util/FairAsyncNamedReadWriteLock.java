...
public class FairAsyncNamedReadWriteLock<T> implements AsyncNamedReadWriteLock<T> {
  ...
  @Override
  public CompletionStage<ReadLockToken> acquireReadLock(final T name) {
    ...
    findHead: while (true) {
      ...
      if (headRef == created.ref) {
        ...
        return CompletableFuture.completedFuture(created.node);
      }
  ...
  @Override
  public CompletionStage<WriteLockToken> acquireWriteLock(final T name) {
    final NodeBox<NamedNode> created = new NodeBox<>();

 ...
 * same thread they are queued and run in a while loop. You could write the previous example like
 * this:
 * 
 * <pre>
...
public final class StackUnroller {
  ...
  private static class StackUnrollerInternal<T> extends CompletableFuture<T> {
    ...
    private StackUnrollerInternal(final Predicate<T> shouldContinue,
        final Function<T, CompletionStage<T>> f, final T initialValue) {
      this.shouldContinue = shouldContinue;
      this.f = f;
      unroll(initialValue, null, null);
    }

    private void unroll(final T completed, final Thread previousThread,
        final PassBack previousPassBack) {
   ...
   *         returns false
   * 
   * @see StackUnroller#asyncDoWhileAsyncInitial(Function, Future, Predicate)
   * @see StackUnroller#asyncWhile(Predicate, Function, Object)
   * @see ExceptionalStackUnroller#asyncDoWhile(Function, Object, Predicate)
   */
  public static <T> CompletionStage<T> asyncDoWhile(final Function<T, CompletionStage<T>> f,
      final T initialValue, final Predicate<T> shouldContinue) {
   ...
   * 
   * Effectively produces f(seed).flatMap(f).flatMap(f)... .flatMap(f) until an value fails the
   * predicate. Note that predicate will be applied on seed (like a while loop, the initial value is
   * tested). For a do/while style see {@link #asyncDoWhile(Function, Object, Predicate)}. For a
   * version that understands {@link Completed} see
   * {@link ExceptionalStackUnroller#asyncWhile(Predicate, Function, Object)}
   * 
   ...
   * 
   * @see #asyncWhileAsyncInitial(Predicate, Function, Future)
   * @see #asyncDoWhile(Function, Object, Predicate)
   * @see #asyncWhile(Supplier)
   * @see ExceptionalStackUnroller#asyncWhile(Predicate, Function, Object)
   */
  public static <T> CompletionStage<T> asyncWhile(final Predicate<T> shouldContinue,
      final Function<T, CompletionStage<T>> f, final T initialValue) {
   ...
   * 
   * @see #asyncWhile(Predicate, Function, Object)
   * @see ExceptionalStackUnroller#asyncWhile(Predicate, Function, Object)
   */

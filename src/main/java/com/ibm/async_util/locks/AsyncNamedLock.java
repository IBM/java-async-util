 ...
 * given name will share exclusivity with other acquisitions of the same name, based on {@link
 * Object#equals(Object) object equality}.
 *
 ...
 *
 * <p>Note that implementations will generally employ an underlying {@link java.util.Map}; as such,
 * the same precautions must be taken regarding mutability of keys (names). Name objects should not
 * change from the time of acquisition to the time of release, with respect to their {@link
 * Object#equals(Object) equality} and {@link Object#hashCode() hash code} semantics. The release
 * methods of the returned {@link AsyncLock.LockToken} may throw a {@link
 * java.util.ConcurrentModificationException} if such a modification is detected.
 *
...
public interface AsyncNamedLock<T> {
   ...
   *
   * <p>The {@link AsyncLock.LockToken} held by the returned future is used to release the lock
   * after it has been acquired and the lock-protected action has completed.
   */
   ...
   *
   * <p>The {@link AsyncLock.LockToken} held by the returned Optional is used to release the lock
   * after it has been acquired and the lock-protected action has completed.
   */
   ...
   *
   * <p>The returned lock is only guaranteed to meet the requirements of {@link AsyncNamedLock}; in

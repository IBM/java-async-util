...
public interface AsyncNamedLock<T> {
  ...
  /**
   * Acquires the lock associated with the given name. If the associated lock is not currently held,
   * the returned stage will be immediately complete. Otherwise, the returned stage will complete
   * when the lock is no longer held by other acquisitions.
   *
   * <p>
   * The {@link AsyncLock.LockToken} held by the returned stage is used to release the lock after
   * it has been acquired and the lock-protected action has completed.
   *
   * @param name to acquire exclusive access for
   * @return A {@link CompletionStage} which will complete with a {@link AsyncLock.LockToken} when the lock
   *         associated with {@code name} has been exclusively acquired
   */

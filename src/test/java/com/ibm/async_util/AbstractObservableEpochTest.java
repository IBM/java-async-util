...
public abstract class AbstractObservableEpochTest {
  ...
  public void testBlockOnActors() {
    ...
    final EpochToken t1 = e.enter().orElseThrow(AssertionError::new);
  ...
  public void testManyEntrants() {
    ...
    final EpochToken[] tokens = IntStream.range(0, count).parallel()
        .mapToObj(ignored -> e.enter().get()).toArray(EpochToken[]::new);

    final AtomicBoolean completed = new AtomicBoolean(false);
    e.terminate().thenAccept(ignored -> completed.set(true));

    // close all but 1
    Arrays.stream(tokens, 1, tokens.length).parallel().forEach(EpochToken::close);
    Assert.assertTrue(e.isTerminated());
  ...
  public void testExceptionOnExcessClose() {
    final ObservableEpoch e = newEpoch();
    final EpochToken t = e.enter().orElseThrow(AssertionError::new);
    t.close();
    t.close();
  }
  ...
  public void testAwaitCompletion() {
...

    {
      ...
      final CompletableFuture<Void> f = e.awaitCompletion().toCompletableFuture();
      Assert.assertFalse(f.isDone());
      final EpochToken token1 = e.enter().orElseThrow(AssertionError::new);
      Assert.assertFalse(f.isDone());
...

    {
      ...
      final CompletableFuture<Void> f = e.awaitCompletion().toCompletableFuture();
      Assert.assertFalse(f.isDone());
      final EpochToken token2 = e.enter().orElseThrow(AssertionError::new);
      Assert.assertFalse(f.isDone());
      Assert.assertFalse(e.awaitCompletion().toCompletableFuture().isDone());
      e.terminate();
      Assert.assertFalse(f.isDone());

...
public abstract class AbstractAsyncNamedReadWriteLockTest extends AbstractAsyncReadWriteLockTest {
  ...
  public final void testReadNameModificationException() {
    ...
    final ReadLockToken readToken = TestUtil.join(lock.acquireReadLock(key));
    key.value = 1;
  ...
  public final void testWriteNameModificationException() {
    final AsyncNamedReadWriteLock<MutableKey> lock = getNamedReadWriteLock();
    final MutableKey key = new MutableKey();
    final WriteLockToken writeToken = TestUtil.join(lock.acquireWriteLock(key));
    key.value = 1;
  ...
  public static abstract class AbstractAsyncNamedReadWriteLockFairnessTest
    ...
    public void testFairness() throws Exception {
      ...
      // under write lock
      final CompletableFuture<ReadLockToken> read1 =
          narwls.acquireReadLock(s).toCompletableFuture();
      final CompletableFuture<ReadLockToken> read2 =
          narwls.acquireReadLock(s).toCompletableFuture();
      final CompletableFuture<ReadLockToken> read3 =
          narwls.acquireReadLock(s).toCompletableFuture();
      ...
      final CompletableFuture<ReadLockToken> read4 =
          narwls.acquireReadLock(s).toCompletableFuture();
      final CompletableFuture<ReadLockToken> read5 =
          narwls.acquireReadLock(s).toCompletableFuture();
      final CompletableFuture<ReadLockToken> read6 =
          narwls.acquireReadLock(s).toCompletableFuture();
      Assert.assertFalse(read4.isDone());
      Assert.assertFalse(read5.isDone());
      Assert.assertFalse(read6.isDone());

      final CompletableFuture<WriteLockToken> write3 =
          narwls.acquireWriteLock(s).toCompletableFuture();
      ...
      final CompletableFuture<ReadLockToken> read7 =
          narwls.acquireReadLock(s).toCompletableFuture();
      final CompletableFuture<WriteLockToken> write4 =
          narwls.acquireWriteLock(s).toCompletableFuture();
      ...
      TestUtil.join(Combinators.all(read4, read5, read6))
